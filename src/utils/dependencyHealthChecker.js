 ...
 * @param {Object} dependencies - Object with service names as keys and full health URLs as values
...
export async function checkDependencyHealth(dependencies, timeout = 5000) {
  ...
  const healthChecks = Object.entries(dependencies).map(async ([serviceName, healthUrl]) => {
    try {
 ...
 * Uses full health endpoint URLs for maximum flexibility
 * @returns {Object} - Object with service names as keys and full health endpoint URLs as values
 */
